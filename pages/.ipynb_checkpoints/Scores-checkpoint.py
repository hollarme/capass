...
try:
    ...
    if score_for == ":red[Defence Score Sheet]":
        ...
        with st.status("", expanded=True) as status:
            ...
            st.sidebar.download_button("Download defence score sheet", dsc, file_name="defence_score_sheet.csv", type="primary", disabled= False if assessor else True)
        ...
        with st.status("", expanded=True) as status:
            with st.container(border=True):
                ...
                st.sidebar.download_button("Download supervisor score sheet", dsc, file_name='supervisor_score_sheet.csv', type="primary", disabled= False if supervisor else True)
